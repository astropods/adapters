...
export class MessagingBridge {
    ...
    async connectWithRetry() {
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                console.log(`Waiting for messaging service (attempt ${attempt}/${MAX_RETRIES}, retry in ${delay}ms)...`);
    ...
    async start() {
        ...
        this.stream.on("response", (response) => {
            ...
            const username = message.user?.username || message.user?.id || "Anonymous";
...
//# sourceMappingURL=messaging-bridge.js.map